...
function wpsc_currency_display( $price_in, $args = null ) {
	...
	if ( ! $query['isocode'] ) {
		...
		// TODO can deprecate this caching because the WPSC_Geography class already caches the data
		if ( ! $wpsc_currency_data = wp_cache_get( $currency_type, 'wpsc_currency_id' ) ) {
			$wpsc_currency_data = WPSC_Geography::currency_data( $currency_type, true );
...
function wpsc_get_currency_symbol(){
	$currency_type = get_option( 'currency_type' );
	$wpsc_currency_data = WPSC_Geography::currency_symbol( $currency_type );
