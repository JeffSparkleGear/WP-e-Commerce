...
function wpsc_add_product_category_default( $product_id ) {
	$terms = get_terms( 'wpsc_product_category', array(
		'orderby'    => 'id',
		'hide_empty' => 0
	) );
	if ( ! empty( $terms ) ) {
...
function wpsc_update_custom_meta( $product_id, $post_data ) {
	...
	if ( ! isset( $post_data['custom_meta'] ) ) {
		$post_data['custom_meta'] = '';
	}
...
function _wpsc_action_refresh_variation_parent_terms( $post_id ) {
	...
	if ( $post->post_type != 'wpsc-product' || ! $post->post_parent || in_array( $post->post_status, array(
			'publish',
			'inherit'
		) ) ) {
		return;
