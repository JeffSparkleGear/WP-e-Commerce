...
//		exit();
	...
	function get_local_currency_code() {
		if ( empty( $this->local_currency_code ) ) {
			global $wpdb;
			$this->local_currency_code = WPSC_Geography::currency_code( get_option( 'currency_type' ) );
		}
...
function wpsc_paypal_express_convert( $amt ) {
	...
	if ( empty( $rate ) ) {
		...
		if ( empty( $local_currency_code ) ) {
			$local_currency_code = WPSC_Geography::currency_code( get_option( 'currency_type' ) );
		}
...
function form_paypal_express() {
	...
	$paypal_ipn = get_option( 'paypal_ipn' );
	$store_currency_code = WPSC_Geography::currency_code( absint( get_option( 'currency_type' ) ) );
	...
	if ( $current_currency != $store_currency_code ) {
		...
		// TODO verify that this query is correct, the WPSC_Geography call that repalced it was coded to duplicate the results, but
		...
		$currency_list = WPSC_Geography::currencies( true );
...
function wpsc_get_paypal_currency_code() {
	global $wpdb, $wpsc_gateways;
	$paypal_currency_code = WPSC_Geography::currency_code( get_option( 'currency_type' ) );
	if ( ! in_array( $paypal_currency_code, $wpsc_gateways['wpsc_merchant_paypal_express']['supported_currencies']['currency_list'] ) )
		$paypal_currency_code = get_option( 'paypal_curcode', 'USD' );
